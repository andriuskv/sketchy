...
export default function Tooltip() {
  ...
  useLayoutEffect(() => {
    ...
    function handleKeyUp(event: KeyboardEvent) {
      ...
      const content = document.querySelector("[data-tooltip-content]")?.cloneNode(true) as HTMLElement;

      if (content) {
        activeTooltips.current.add(tooltipElement);
        tooltipElement.addEventListener("blur", resetTooltipElement, { once: true });
        timeoutId.current = timeout(() => {
          const tooltip: Tooltip = {
            id: getRandomString(),
            content,
  ...
  return (
    <>
      {tooltips.map(tooltip => (
        <div id={tooltip.id} role="tooltip" className="tooltip" style={{ "positionAnchor": `--anchor-${tooltip.id}` } as CSSProperties} key={tooltip.id} dangerouslySetInnerHTML={{ __html: tooltip.content.innerHTML }}>
        </div>
      ))}
    </>
  );
